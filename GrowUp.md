...
JDBC 驱动通常会在 JAR 包中包含一个
 `META-INF/services/java.sql.Driver` 文件，里面写着实现类的全名。
 JVM 启动时，`ServiceLoader` 会用 **线程上下文类加载器（ContextClassLoader）** 来加载这些驱动类。

2. 线程上下文类加载器（ContextClassLoader）

`DriverManager` 并不直接用自己的类加载器（引导类加载器）加载驱动类，而是：

```
ClassLoader cl = Thread.currentThread().getContextClassLoader();
```

再通过这个类加载器加载用户的 JDBC 驱动类。
 这样就实现了“父加载器反向使用子加载器的类” → **打破双亲委派**。
...
现象：请求一个根本不存在的数据，那么每次请求都打到数据库

解决：

1. 缓存空值 短失效时间
2. 使用布隆过滤器，先把数据都放进布隆过滤器然后请求到来先判断是否存在（允许误判）

### 缓存雪崩

RedisKey集中失效，大量并发请求打到数据库

1. 分散key的失效时间可以在统一设计的时间值上加一个随机数
2. 缓存预热，提前将数据加载到缓存中
...
​	看门狗机制是如何判断线程还在执行的？检查key vlaue中的线程ID仍然等于当前线程的标识即可
...
1. **服务启动**：每当一个服务启动时，它会通过 Nacos 提供的 API 向 Nacos 服务器发送服务注册请求，告知服务器它的 **IP 地址**、**端口号**、**服务名称** 等信息。
2. **Nacos注册中心**：Nacos 服务器接收到注册请求后，会将该服务的信息保存到数据库中，供其他服务查询。
3. **服务心跳**：为了保证服务的健康状态，注册的服务会定期向 Nacos 发送 **心跳**，以告知自己仍然存活。如果超过一定时间未收到心跳，Nacos 会将该服务从服务注册表中移除。

#### 服务发现工作原理

1. **服务消费方**：服务消费者通过调用 Nacos 提供的 **API** 或 **SDK** 向 Nacos 发送查询请求，获取目标服务的地址列表。
2. **Nacos 返回地址**：Nacos 返回目标服务的 **IP 地址** 和 **端口号**。消费者可以直接通过这些信息与目标服务进行通信。
3. **动态监听**：Nacos 支持服务的动态监听，当目标服务的地址发生变化时，消费者能够及时感知并重新获取最新的服务列表。
...
1. 生产者确认机制，确保消息成功写入MQ，Mandatory保证消息无法路由到队列时返回给生产者
2. 开启队列及消息持久化
